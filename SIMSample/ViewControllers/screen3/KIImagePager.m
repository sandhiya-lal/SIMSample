...
@interface KIImagePager () <UIScrollViewDelegate , UIGestureRecognizerDelegate>
{
...
{
	_captionLabel = [[UILabel alloc] initWithFrame:CGRectMake(_scrollView.frame.size.width/2, _scrollView.frame.size.height/2, 60, 30)];
	[_captionLabel setCenter:_scrollView.center];
    ...
    [_captionLabel setFont:self.captionFont];
	 [_captionLabel setText:[NSString stringWithFormat:@"copied" ]];
...
- (void) loadData
{
    ...
    if([aImageUrls count] > 0) {
        ...
        for (int i = 0; i < [aImageUrls count]; i++) {
			...
			UILongPressGestureRecognizer *longPressGestur
			= [[UILongPressGestureRecognizer alloc]
			   initWithTarget:self action:@selector(handleLongPress:)];
			longPressGestur.delegate = self;
			longPressGestur.delaysTouchesBegan = YES;
			[imageView addGestureRecognizer:longPressGestur];
...
- (void)handleLongPress:(UILongPressGestureRecognizer *)sender
{
	...
	if ([_delegate respondsToSelector:@selector(imagePager:didLongPressImageAtIndex:)]) {
		[_delegate imagePager:self didLongPressImageAtIndex:[(UILongPressGestureRecognizer *)sender view].tag];
	}
}

- (void)handlePinchGesture:(UIPinchGestureRecognizer *)sender
{
	[self handlePinchGesturefour:sender];
}

- (void)handlePinchGesturefour:(UIPinchGestureRecognizer *)gestureRecognizer {
